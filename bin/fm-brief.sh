...
The task root in \`FM_TASK_WORKTREE_WSL\` is authoritative for repository operations; do not re-resolve the project path or substitute the primary checkout.
Linux-native tools must use \`FM_TASK_WORKTREE_WSL\`, and Windows-native tools may use \`FM_TASK_WORKTREE_WINDOWS\` only when it is non-empty and must never pass that Windows spelling to Git or Treehouse.
